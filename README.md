...
모바일 센서 API는 HTTPS 환경에서 더 안정적으로 작동하므로 GitHub Pages 배포를 권장합니다.
...
1. GitHub에서 `earthquake-sensor-prototype` 저장소를 만듭니다.
2. `index.html`, `style.css`, `script.js`, `README.md` 파일을 저장소에 올립니다.
3. 저장소의 `Settings` → `Pages` 메뉴로 이동합니다.
4. `Deploy from a branch`를 선택합니다.
5. branch는 `main`, folder는 `/root`로 설정합니다.
6. 몇 분 뒤 아래 형식의 HTTPS 주소로 접속합니다.
...
## 12. 실제 안드로이드 지진 경보 시스템과 내 프로토타입의 차이
